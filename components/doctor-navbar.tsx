...
import { CalendarDays, Clock, MessageSquare, User, LogOut, Menu, X, LayoutDashboard } from "lucide-react"
...
export function DoctorNavbar() {
  ...
  const handleLogout = async () => {
    try {
      ...
      localStorage.removeItem("authToken") // Cleanup just in case
      router.refresh()
      ...
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between">
          ...
          <Link href="/doctor/dashboard" className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground bg-secondary px-2 py-0.5 rounded">Doctor</span>
          </Link>
              ...
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="flex items-center gap-2 px-3">
                  ...
                  <span className="hidden sm:inline text-sm font-medium text-foreground">
                    {user ? `Dr. ${user.first_name}` : "Account"}
                  </span>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56 glass-card bg-background/95 backdrop-blur-xl border-border/50">
                <div className="px-2 py-1.5">
                  <p className="text-sm font-medium text-foreground">
                    {user ? `Dr. ${user.first_name} ${user.last_name}` : "Guest"}
                  </p>
                  <p className="text-xs text-muted-foreground">{user?.email}</p>
                </div>
                <DropdownMenuSeparator />
                <DropdownMenuItem asChild>
                  <Link href="/doctor/profile" className="flex items-center gap-2 cursor-pointer">
